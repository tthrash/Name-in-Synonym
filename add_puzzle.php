  ...
  <div class="header">
	...
	<?PHP
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			...
			echo '<form action="add_puzzle.php" method="post">
		...
		function create_puzzle($size, $word) {
			$table = "";
			$table .= "<h1>Enter the words and clues for <div class='red'>" . $word . "</div></h1>";
			$table .= "<table><thead><tr><th>No</th><th>Character</th><th>Synonym (word)</th><th>Clue</th></thead>";
			for ($i = 0; $i < $size; $i++) {
				if ($i == 0) {
					$table .= "<tbody>";
				}
				$char = substr($word, $i, 1);
				$table .= "<tr><td>" . ($i + 1) . "</td><td>" . strtoupper($char) . "</td><td><input type='text'/></td><td><input type='text'/></td></tr>";
			}
			$table .= "</tbody></table>";
			return $table;
		}
