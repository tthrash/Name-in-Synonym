<?php
	...
	function create_puzzle_words($name)
	{
			...
			}else
			{
				...
				$wordChars = getCharArray($name);
				$wordLng = count($wordChars);
				for($i = 0; $i < $wordLng; $i++)
				{
					$character = $wordChars[$i];
...
>>>>>>> 4091ee607dd4cdce873e7848de4799ff918f9129
		$result =  $db->query($sql);
		//echo '<p>'. $result . '</p>';
	}
	...
	function insertIntoCharacters($word_id) {
		...
		if ($num_rows == 0)
		{
			...
			$wordChars = getCharArray($word_value);
			$wordLng = count($wordChars);
			for ($i = 0; $i < $wordLng; $i++)
			{
				$sql = 'INSERT INTO characters (word_id, character_index, character_value) VALUES (\'' . $word_id  . '\', \'' . $i . '\', \'' . $wordChars[$i] . '\');';
				$result =  $db->query($sql);
			}
		}
	...
	function getWordLength($word)
	{
		...
		$wordChars = $letters->parseToLogicalChars($word, "Telugu");
		return count($wordChars);
	}
	...
	function insertWordsAndCharacter($listOfWords)
	{
		for($i = 0; $i < count($listOfWords);$i++){
			...
			{ 
				...
				$wordChars = getCharArray($listOfWords[$i]);
				$wordLng = count($wordChars);
				echo 'Num: '.$wordLng. '; '.$listOfWords[$i]. '; Index: '.$i.'; Sum: ';
				$sum = "";
				for($j = 0; $j < $wordLng; $j++) {
					$sum .= $wordChars[$j];
					//insert each letter into char table.
					$sqlAddLetters = 'INSERT INTO characters (word_id, character_index, character_value) VALUES (\''. $word_id . '\', \'' . $j .'\', \''. $wordChars[$j].'\');';
					$result =  $db->query($sqlAddLetters);
/* 					if(!$result)
					{
						echo"Inserting character failed!" . $db->error;				    
					} */
				}
				echo $sum.'<br>';
			}
